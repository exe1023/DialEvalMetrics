...
def train(train_x, val_x, test_x, train_y, val_y, test_y, init_mean, init_range, learning_rate=0.01, num_epochs=100, \
    ...
    predictions = []
    predictions.append(
        ("Context", "GT response", "Model response", "ADEM prediction", "Human score", "BLEU2 score", "ROUGE score"))
    for i in range(len(test_y)):
        predictions.append((test_contexts[i], test_gtresponses[i], test_modelresponses[i], model_out[i], test_y[i],
                            bleu2_list[i], rouge_list[i]))
...
if __name__ == '__main__':
    ...
    bad_len = []
    good_len = []
    for i in range(len(twitter_human_scores)):
    ...
    twitter_dialogue_embeddings, aux_features = preprocess_data(twitter_contexts, twitter_gtresponses,
                                                                twitter_modelresponses, context_embedding_file, \
    ...
    if test_liu_data:
        ...
        twitter_dialogue_embeddings_liu, liu_feat = preprocess_data(twitter_contexts_liu, twitter_gtresponses_liu,
