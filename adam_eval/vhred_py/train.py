    ...
    while (step < state['loop_iters'] and
        ...
        if state['utterance_decoder_gating'].upper() == "GRU":
            if (step % 200 == 0) and (model.add_latent_gaussian_per_utterance):
                ...
                print('mean gradients_wrt_softmax', numpy.mean(numpy.abs(numpy.mean(gradients_wrt_softmax, axis=0))),
                      numpy.mean(gradients_wrt_softmax, axis=0))
                print('std gradients_wrt_softmax', numpy.mean(numpy.std(gradients_wrt_softmax, axis=0)),
                ...
                if 'latent_gaussian_linear_dynamics' in state:
                    if state['latent_gaussian_linear_dynamics']:
                        ...
                        print('prior_Wl_linear_dynamics', numpy.sum(numpy.abs(prior_Wl_linear_dynamics)),
                        ...
                        print('approx_posterior_Wl_linear_dynamics',
