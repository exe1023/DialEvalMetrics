...
class DailyDialogProcessor(DataProcessor):
    ...
    def _create_examples(samples_list, set_type, model_maxlen):
        ...
        for i in range(len(samples_list)):
            ...
            if len(text.split(' ')) > model_maxlen:
                list_ = text.split(' ')[:model_maxlen]
                text = ' '.join(list_)
            raw_text = text                    

            if len(ctx.split(' ')) > model_maxlen:
                list_ = ctx.split(' ')[:model_maxlen]
                ctx = ' '.join(list_)
            raw_context = ctx              

            if len(response.split(' ')) > model_maxlen:
                list_ = response.split(' ')[:model_maxlen]
                response = ' '.join(list_)
            raw_response = response            

            raw_text = tx.utils.compat_as_text(raw_text)
            raw_context = tx.utils.compat_as_text(raw_context)
            raw_response = tx.utils.compat_as_text(raw_response)
