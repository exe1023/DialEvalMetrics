...
class LinearCombination(EncoderDecoderBase):
    ...
    def init_params(self, cond_size, output_size, force_min_max_intervals, min_val, max_val):
        self.W = add_to_params(self.params,
                               theano.shared(value=np.ones((output_size,), dtype='float32'), name='W_x' + self.name))
...
class TwoLayerMLP(EncoderDecoderBase):
    def init_params(self, inp_size, hidden_size, output_size):
        # First layer
        self.W1_in_tanh = add_to_params(self.params, theano.shared(value=NormalInit(self.rng, inp_size, hidden_size),
                                                                   name='W1_in_' + self.name))
        self.b1_in_tanh = add_to_params(self.params, theano.shared(value=np.zeros((hidden_size,), dtype='float32'),
                                                                   name='b1_in_' + self.name))
        self.W1_in_skip = add_to_params(self.params, theano.shared(value=NormalInit(self.rng, inp_size, hidden_size),
                                                                   name='W1_in_skip_' + self.name))
        self.b1_in_skip = add_to_params(self.params, theano.shared(value=np.zeros((hidden_size,), dtype='float32'),
                                                                   name='b1_in_skip_' + self.name))

        # First layer batch norm / layer norm parameters
        self.normop_in_tanh_h1_gamma = add_to_params(self.params, theano.shared(
            value=self.normop_gamma_init * np.ones((hidden_size,), dtype='float32'),
            name='normop_in_tanh_h1_gamma_' + self.name))
        self.normop_in_tanh_h1_mean = add_to_params(self.params,
                                                    theano.shared(value=np.zeros((hidden_size,), dtype='float32'),
                                                                  name='normop_in_tanh_h1_mean_' + self.name))
        self.normop_in_tanh_h1_var = add_to_params(self.params, theano.shared(
            value=(1e-7) * np.ones((hidden_size,), dtype='float32'), name='normop_in_tanh_h1_var_' + self.name))

        self.normop_in_skip_h1_gamma = add_to_params(self.params, theano.shared(
            value=self.normop_gamma_init * np.ones((hidden_size,), dtype='float32'),
            name='normop_in_skip_h1_gamma_' + self.name))
        self.normop_in_skip_h1_mean = add_to_params(self.params,
                                                    theano.shared(value=np.zeros((hidden_size,), dtype='float32'),
                                                                  name='normop_in_skip_h1_mean_' + self.name))
        self.normop_in_skip_h1_var = add_to_params(self.params, theano.shared(
            value=(1e-7) * np.ones((hidden_size,), dtype='float32'), name='normop_in_skip_h1_var_' + self.name))

        # Second layer
        self.W2_in_tanh = add_to_params(self.params, theano.shared(value=NormalInit(self.rng, hidden_size, output_size),
                                                                   name='W2_in_' + self.name))
        self.b2_in_tanh = add_to_params(self.params, theano.shared(value=np.zeros((output_size,), dtype='float32'),
                                                                   name='b2_in_' + self.name))

        self.W2_in_skip = add_to_params(self.params, theano.shared(value=NormalInit(self.rng, hidden_size, output_size),
                                                                   name='W2_in_skip_' + self.name))
        self.b2_in_skip = add_to_params(self.params, theano.shared(value=np.zeros((output_size,), dtype='float32'),
                                                                   name='b2_in_skip_' + self.name))

        # Second layer batch norm / layer norm parameters
        self.normop_in_tanh_h2_gamma = add_to_params(self.params, theano.shared(
            value=self.normop_gamma_init * np.ones((output_size,), dtype='float32'),
            name='normop_in_tanh_h2_gamma_' + self.name))
        self.normop_in_tanh_h2_mean = add_to_params(self.params,
                                                    theano.shared(value=np.zeros((output_size,), dtype='float32'),
                                                                  name='normop_in_tanh_h2_mean_' + self.name))
        self.normop_in_tanh_h2_var = add_to_params(self.params, theano.shared(
            value=(1e-7) * np.ones((output_size,), dtype='float32'), name='normop_in_tanh_h2_var_' + self.name))

        self.normop_in_skip_h2_gamma = add_to_params(self.params, theano.shared(
            value=self.normop_gamma_init * np.ones((output_size,), dtype='float32'),
            name='normop_in_skip_h2_gamma_' + self.name))
        self.normop_in_skip_h2_mean = add_to_params(self.params,
                                                    theano.shared(value=np.zeros((output_size,), dtype='float32'),
                                                                  name='normop_in_skip_h2_mean_' + self.name))
        self.normop_in_skip_h2_var = add_to_params(self.params, theano.shared(
            value=(1e-7) * np.ones((output_size,), dtype='float32'), name='normop_in_skip_h2_var_' + self.name))
    ...
    def build_output(self, inp, bnmask):
        ...
        h_linear_inp, h_linear_inp_mean, h_linear_inp_var = NormalizationOperator(self.normop_type,
                                                                                  T.dot(inp, self.W1_in_skip),
                                                                                  self.normop_in_tanh_h1_gamma, bnmask,
                                                                                  self.normop_in_tanh_h1_mean,
                                                                                  self.normop_in_tanh_h1_var)

        h_nonlinear_inp, h_nonlinear_inp_mean, h_nonlinear_inp_var = NormalizationOperator(self.normop_type, T.dot(inp,
                                                                                                                   self.W1_in_tanh) + self.b1_in_tanh,
                                                                                           self.normop_in_skip_h1_gamma,
                                                                                           bnmask,
                                                                                           self.normop_in_skip_h1_mean,
                                                                                           self.normop_in_skip_h1_var)
        ...
        h2_linear_inp, h2_linear_inp_mean, h2_linear_inp_var = NormalizationOperator(self.normop_type,
                                                                                     T.dot(h, self.W2_in_skip),
                                                                                     self.normop_in_skip_h2_gamma,
                                                                                     bnmask,
                                                                                     self.normop_in_skip_h2_mean,
                                                                                     self.normop_in_skip_h2_var)
        h2_nonlinear_inp, h2_nonlinear_inp_mean, h2_nonlinear_inp_var = NormalizationOperator(self.normop_type, T.dot(h,
                                                                                                                      self.W2_in_tanh) + self.b2_in_tanh,
                                                                                              self.normop_in_tanh_h2_gamma,
                                                                                              bnmask,
                                                                                              self.normop_in_tanh_h2_mean,
                                                                                              self.normop_in_tanh_h2_var)
        ...
        if self.normop_type == 'BN':
            ...
            vars_to_update = [self.normop_in_tanh_h1_mean, self.normop_in_tanh_h1_var, self.normop_in_skip_h1_mean,
                              self.normop_in_skip_h1_var, self.normop_in_skip_h2_mean, self.normop_in_skip_h2_var,
                              self.normop_in_tanh_h2_mean, self.normop_in_tanh_h2_var]
            vars_estimates = [h_linear_inp_mean, h_linear_inp_var, h_nonlinear_inp_mean, h_nonlinear_inp_var,
                              h2_linear_inp_mean, h2_linear_inp_var, h2_nonlinear_inp_mean, h2_nonlinear_inp_var]
...
class UtteranceEncoder(EncoderDecoderBase):
    ...
    def init_params(self, word_embedding_param):
        ...
        self.W_in = add_to_params(self.params,
                                  theano.shared(value=NormalInit(self.rng, self.rankdim, self.qdim_encoder),
                                                name='W_in_' + self.name))
        self.W_hh = add_to_params(self.params,
                                  theano.shared(value=OrthogonalInit(self.rng, self.qdim_encoder, self.qdim_encoder),
                                                name='W_hh_' + self.name))
        self.b_hh = add_to_params(self.params, theano.shared(value=np.zeros((self.qdim_encoder,), dtype='float32'),
                                                             name='b_hh_' + self.name))

        # Initialize batch norm / layer norm parameters
        self.normop_in_h_gamma = add_to_params(self.params, theano.shared(
            value=self.normop_gamma_init * np.ones((self.qdim_encoder,), dtype='float32'),
            name='normop_in_h_gamma_' + self.name))
        self.normop_in_h_mean = add_to_params(self.params, theano.shared(
            value=np.zeros((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
            name='normop_in_h_mean_' + self.name))
        self.normop_in_h_var = add_to_params(self.params, theano.shared(
            value=(1e-7) * np.ones((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
            name='normop_in_h_var_' + self.name))

        self.normop_in_x_gamma = add_to_params(self.params, theano.shared(
            value=self.normop_gamma_init * np.ones((self.qdim_encoder,), dtype='float32'),
            name='normop_in_x_gamma_' + self.name))
        self.normop_in_x_mean = add_to_params(self.params, theano.shared(
            value=np.zeros((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
            name='normop_in_x_mean_' + self.name))
        self.normop_in_x_var = add_to_params(self.params, theano.shared(
            value=(1e-7) * np.ones((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
            name='normop_in_x_var_' + self.name))

        if self.utterance_encoder_gating == "GRU":
            self.W_in_r = add_to_params(self.params,
                                        theano.shared(value=NormalInit(self.rng, self.rankdim, self.qdim_encoder),
                                                      name='W_in_r_' + self.name))
            self.W_in_z = add_to_params(self.params,
                                        theano.shared(value=NormalInit(self.rng, self.rankdim, self.qdim_encoder),
                                                      name='W_in_z_' + self.name))
            self.W_hh_r = add_to_params(self.params, theano.shared(
                value=OrthogonalInit(self.rng, self.qdim_encoder, self.qdim_encoder), name='W_hh_r_' + self.name))
            self.W_hh_z = add_to_params(self.params, theano.shared(
                value=OrthogonalInit(self.rng, self.qdim_encoder, self.qdim_encoder), name='W_hh_z_' + self.name))
            ...
            # Initialize batch norm / layer norm parameters
            self.normop_r_h_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.qdim_encoder,), dtype='float32'),
                name='normop_r_h_gamma_' + self.name))
            self.normop_r_h_mean = add_to_params(self.params, theano.shared(
                value=np.zeros((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
                name='normop_r_h_mean_' + self.name))
            self.normop_r_h_var = add_to_params(self.params, theano.shared(
                value=(1e-7) * np.ones((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
                name='normop_r_h_var_' + self.name))

            self.normop_r_x_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.qdim_encoder,), dtype='float32'),
                name='normop_r_x_gamma_' + self.name))
            self.normop_r_x_mean = add_to_params(self.params, theano.shared(
                value=np.zeros((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
                name='normop_r_x_mean_' + self.name))
            self.normop_r_x_var = add_to_params(self.params, theano.shared(
                value=(1e-7) * np.ones((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
                name='normop_r_x_var_' + self.name))

            self.normop_z_h_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.qdim_encoder,), dtype='float32'),
                name='normop_z_h_gamma_' + self.name))
            self.normop_z_h_mean = add_to_params(self.params, theano.shared(
                value=np.zeros((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
                name='normop_z_h_mean_' + self.name))
            self.normop_z_h_var = add_to_params(self.params, theano.shared(
                value=(1e-7) * np.ones((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
                name='normop_z_h_var_' + self.name))

            self.normop_z_x_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.qdim_encoder,), dtype='float32'),
                name='normop_z_x_gamma_' + self.name))
            self.normop_z_x_mean = add_to_params(self.params, theano.shared(
                value=np.zeros((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
                name='normop_z_x_mean_' + self.name))
            self.normop_z_x_var = add_to_params(self.params, theano.shared(
                value=(1e-7) * np.ones((self.normop_max_enc_seq, self.qdim_encoder), dtype='float32'),
                name='normop_z_x_var_' + self.name))
    ...
    def GRU_step(self, x_t, m_t, bnmask_t, *args):
        ...
        # Compute reset gate
        r_t_normop_x_inp, r_t_normop_x_mean, r_t_normop_x_var = NormalizationOperator(self.normop_type,
                                                                                      T.dot(x_t, self.W_in_r),
                                                                                      self.normop_r_x_gamma, bnmask_t,
                                                                                      self.normop_r_x_mean[
                                                                                      n_t_truncated, :],
                                                                                      self.normop_r_x_var[n_t_truncated,
                                                                                      :])
        r_t_normop_h_inp, r_t_normop_h_mean, r_t_normop_h_var = NormalizationOperator(self.normop_type,
                                                                                      T.dot(hr_tm1, self.W_hh_r),
                                                                                      self.normop_r_h_gamma, bnmask_t,
                                                                                      self.normop_r_h_mean[
                                                                                      n_t_truncated, :],
                                                                                      self.normop_r_h_var[n_t_truncated,
                                                                                      :])
        r_t = T.nnet.sigmoid(r_t_normop_x_inp + r_t_normop_h_inp + self.b_r)

        # Compute update gate
        z_t_normop_x_inp, z_t_normop_x_mean, z_t_normop_x_var = NormalizationOperator(self.normop_type,
                                                                                      T.dot(x_t, self.W_in_z),
                                                                                      self.normop_z_x_gamma, bnmask_t,
                                                                                      self.normop_z_x_mean[
                                                                                      n_t_truncated, :],
                                                                                      self.normop_z_x_var[n_t_truncated,
                                                                                      :])
        z_t_normop_h_inp, z_t_normop_h_mean, z_t_normop_h_var = NormalizationOperator(self.normop_type,
                                                                                      T.dot(hr_tm1, self.W_hh_z),
                                                                                      self.normop_z_h_gamma, bnmask_t,
                                                                                      self.normop_z_h_mean[
                                                                                      n_t_truncated, :],
                                                                                      self.normop_z_h_var[n_t_truncated,
                                                                                      :])
        z_t = T.nnet.sigmoid(z_t_normop_x_inp + z_t_normop_h_inp + self.b_z)

        # Compute h_tilde
        h_tilde_normop_x_inp, h_tilde_normop_x_mean, h_tilde_normop_x_var = NormalizationOperator(self.normop_type,
                                                                                                  T.dot(x_t, self.W_in),
                                                                                                  self.normop_in_x_gamma,
                                                                                                  bnmask_t,
                                                                                                  self.normop_in_x_mean[
                                                                                                  n_t_truncated, :],
                                                                                                  self.normop_in_x_var[
                                                                                                  n_t_truncated, :])

        h_tilde_normop_h_inp, h_tilde_normop_h_mean, h_tilde_normop_h_var = NormalizationOperator(self.normop_type,
                                                                                                  T.dot(r_t * hr_tm1,
                                                                                                        self.W_hh),
                                                                                                  self.normop_in_h_gamma,
                                                                                                  bnmask_t,
                                                                                                  self.normop_in_h_mean[
                                                                                                  n_t_truncated, :],
                                                                                                  self.normop_in_h_var[
                                                                                                  n_t_truncated, :])
        ...
        return [h_t, T.cast(new_n_t, 'int8'), r_t, z_t, h_tilde, r_t_normop_x_mean, r_t_normop_x_var, r_t_normop_h_mean,
                r_t_normop_h_var, z_t_normop_x_mean, z_t_normop_x_var, z_t_normop_h_mean, z_t_normop_h_var,
                h_tilde_normop_x_mean, h_tilde_normop_x_var, h_tilde_normop_h_mean, h_tilde_normop_h_var]
    ...
    def build_encoder(self, x, xmask=None, bnmask=None, prev_state=None, **kwargs):
        ...
        if self.utterance_encoder_gating == "GRU":
            f_enc = self.GRU_step
            o_enc_info = [h_0, n_0, None, None, None, None, None, None, None, None, None, None, None, None, None, None,
                          None]
        else:
        ...
        if self.utterance_encoder_gating == 'GRU':
            ...
            if self.normop_type == 'BN':
                if (not one_step) and (x.ndim == 2) and (bnmask_given):
                    ...
                    vars_to_update = [self.normop_r_x_mean, self.normop_r_x_var, self.normop_r_h_mean,
                                      self.normop_r_h_var, self.normop_z_x_mean, self.normop_z_x_var,
                                      self.normop_z_h_mean, self.normop_z_h_var, self.normop_in_x_mean,
                                      self.normop_in_x_var, self.normop_in_h_mean, self.normop_in_h_var]
...
class DialogEncoder(EncoderDecoderBase):
    ...
    def init_params(self):
        ...
        if self.deep_dialogue_input:
            ...
            self.input_mlp = TwoLayerMLP(self.state, self.rng, input_dim, self.sdim, self.sdim, self,
                                         '_input_mlp_' + self.name)
        ...
        if self.dialogue_encoder_gating == "GRU":
            ...
            self.Ws_in_overwrite = add_to_params(self.params, theano.shared(
                value=NormalInit(self.rng, transformed_input_dim, self.sdim), name='Ws_in_overwrite' + self.name))
            self.bs_overwrite = add_to_params(self.params, theano.shared(value=np.zeros((self.sdim,), dtype='float32'),
                                                                         name='bs_overwrite' + self.name))

            # Gating mechanism defining whether to overwrite or not
            self.Ws_in_o = add_to_params(self.params,
                                         theano.shared(value=NormalInit(self.rng, transformed_input_dim, self.sdim),
                                                       name='Ws_in_o' + self.name))
            self.Ws_hh_o = add_to_params(self.params,
                                         theano.shared(value=OrthogonalInit(self.rng, self.sdim, self.sdim),
                                                       name='Ws_hh_o' + self.name))
            self.bs_o = add_to_params(self.params, theano.shared(value=np.zeros((self.sdim,), dtype='float32'),
                                                                 name='bs_o' + self.name))

            # Batch norm parameters
            self.normop_in_hs_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.sdim,), dtype='float32'),
                name='normop_in_hs_gamma' + self.name))
            self.normop_in_hs_mean = add_to_params(self.params,
                                                   theano.shared(value=np.zeros((self.sdim,), dtype='float32'),
                                                                 name='normop_in_hs_mean' + self.name))
            self.normop_in_hs_var = add_to_params(self.params,
                                                  theano.shared(value=(1e-7) * np.ones((self.sdim,), dtype='float32'),
                                                                name='normop_in_hs_var' + self.name))

            self.normop_in_h_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.sdim,), dtype='float32'),
                name='normop_in_h_gamma' + self.name))
            self.normop_in_h_mean = add_to_params(self.params,
                                                  theano.shared(value=np.zeros((self.sdim,), dtype='float32'),
                                                                name='normop_in_h_mean' + self.name))
            self.normop_in_h_var = add_to_params(self.params,
                                                 theano.shared(value=(1e-7) * np.ones((self.sdim,), dtype='float32'),
                                                               name='normop_in_h_var' + self.name))

            self.normop_rs_hs_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.sdim,), dtype='float32'),
                name='normop_rs_hs_gamma' + self.name))
            self.normop_rs_hs_mean = add_to_params(self.params,
                                                   theano.shared(value=np.zeros((self.sdim,), dtype='float32'),
                                                                 name='normop_rs_hs_mean' + self.name))
            self.normop_rs_hs_var = add_to_params(self.params,
                                                  theano.shared(value=(1e-7) * np.ones((self.sdim,), dtype='float32'),
                                                                name='normop_rs_hs_var' + self.name))

            self.normop_rs_h_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.sdim,), dtype='float32'),
                name='normop_rs_h_gamma' + self.name))
            self.normop_rs_h_mean = add_to_params(self.params,
                                                  theano.shared(value=np.zeros((self.sdim,), dtype='float32'),
                                                                name='normop_rs_h_mean' + self.name))
            self.normop_rs_h_var = add_to_params(self.params,
                                                 theano.shared(value=(1e-7) * np.ones((self.sdim,), dtype='float32'),
                                                               name='normop_rs_h_var' + self.name))

            self.normop_zs_hs_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.sdim,), dtype='float32'),
                name='normop_zs_hs_gamma' + self.name))
            self.normop_zs_hs_mean = add_to_params(self.params,
                                                   theano.shared(value=np.zeros((self.sdim,), dtype='float32'),
                                                                 name='normop_zs_hs_mean' + self.name))
            self.normop_zs_hs_var = add_to_params(self.params,
                                                  theano.shared(value=(1e-7) * np.ones((self.sdim,), dtype='float32'),
                                                                name='normop_zs_hs_var' + self.name))

            self.normop_zs_h_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.sdim,), dtype='float32'),
                name='normop_zs_h_gamma' + self.name))
            self.normop_zs_h_mean = add_to_params(self.params,
                                                  theano.shared(value=np.zeros((self.sdim,), dtype='float32'),
                                                                name='normop_zs_h_mean' + self.name))
            self.normop_zs_h_var = add_to_params(self.params,
                                                 theano.shared(value=(1e-7) * np.ones((self.sdim,), dtype='float32'),
                                                               name='normop_zs_h_var' + self.name))

            self.normop_os_hs_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.sdim,), dtype='float32'),
                name='normop_os_hs_gamma' + self.name))
            self.normop_os_hs_mean = add_to_params(self.params,
                                                   theano.shared(value=np.zeros((self.sdim,), dtype='float32'),
                                                                 name='normop_os_hs_mean' + self.name))
            self.normop_os_hs_var = add_to_params(self.params,
                                                  theano.shared(value=(1e-7) * np.ones((self.sdim,), dtype='float32'),
                                                                name='normop_os_hs_var' + self.name))

            self.normop_os_h_gamma = add_to_params(self.params, theano.shared(
                value=self.normop_gamma_init * np.ones((self.sdim,), dtype='float32'),
                name='normop_os_h_gamma' + self.name))
            self.normop_os_h_mean = add_to_params(self.params,
                                                  theano.shared(value=np.zeros((self.sdim,), dtype='float32'),
                                                                name='normop_os_h_mean' + self.name))
            self.normop_os_h_var = add_to_params(self.params,
                                                 theano.shared(value=(1e-7) * np.ones((self.sdim,), dtype='float32'),
                                                               name='normop_os_h_var' + self.name))
    ...
    def GRU_dialogue_step(self, h_t, m_t, bnmask_t, hs_tm1, *args):
        ...
        rs_t_normop_h_inp, rs_t_normop_h_mean, rs_t_normop_h_var = NormalizationOperator(self.normop_type,
                                                                                         T.dot(h_t, self.Ws_in_r),
                                                                                         self.normop_rs_h_gamma,
                                                                                         bnmask_t,
                                                                                         self.normop_rs_h_mean,
                                                                                         self.normop_rs_h_var)
        rs_t_normop_hs_inp, rs_t_normop_hs_mean, rs_t_normop_hs_var = NormalizationOperator(self.normop_type,
                                                                                            T.dot(hs_tm1, self.Ws_hh_r),
                                                                                            self.normop_rs_hs_gamma,
                                                                                            bnmask_t,
                                                                                            self.normop_rs_hs_mean,
                                                                                            self.normop_rs_hs_var)
        ...
        zs_t_normop_h_inp, zs_t_normop_h_mean, zs_t_normop_h_var = NormalizationOperator(self.normop_type,
                                                                                         T.dot(h_t, self.Ws_in_z),
                                                                                         self.normop_zs_h_gamma,
                                                                                         bnmask_t,
                                                                                         self.normop_zs_h_mean,
                                                                                         self.normop_zs_h_var)
        zs_t_normop_hs_inp, zs_t_normop_hs_mean, zs_t_normop_hs_var = NormalizationOperator(self.normop_type,
                                                                                            T.dot(hs_tm1, self.Ws_hh_z),
                                                                                            self.normop_zs_hs_gamma,
                                                                                            bnmask_t,
                                                                                            self.normop_zs_hs_mean,
                                                                                            self.normop_zs_hs_var)
        ...
        os_t_normop_h_inp, os_t_normop_h_mean, os_t_normop_h_var = NormalizationOperator(self.normop_type,
                                                                                         T.dot(h_t, self.Ws_in_o),
                                                                                         self.normop_os_h_gamma,
                                                                                         bnmask_t,
                                                                                         self.normop_os_h_mean,
                                                                                         self.normop_os_h_var)
        os_t_normop_hs_inp, os_t_normop_hs_mean, os_t_normop_hs_var = NormalizationOperator(self.normop_type,
                                                                                            T.dot(hs_tm1, self.Ws_hh_o),
                                                                                            self.normop_os_hs_gamma,
                                                                                            bnmask_t,
                                                                                            self.normop_os_hs_mean,
                                                                                            self.normop_os_hs_var)
        ...
        hs_tilde_normop_h_inp, hs_tilde_normop_h_mean, hs_tilde_normop_h_var = NormalizationOperator(self.normop_type,
                                                                                                     T.dot(h_t,
                                                                                                           self.Ws_in),
                                                                                                     self.normop_in_h_gamma,
                                                                                                     bnmask_t,
                                                                                                     self.normop_in_h_mean,
                                                                                                     self.normop_in_h_var)
        hs_tilde_normop_hs_inp, hs_tilde_normop_hs_mean, hs_tilde_normop_hs_var = NormalizationOperator(
            self.normop_type, T.dot(rs_t * hs_tm1, self.Ws_hh), self.normop_in_hs_gamma, bnmask_t,
            self.normop_in_hs_mean, self.normop_in_hs_var)
    ...
    def build_encoder(self, h, x, xmask=None, bnmask=None, prev_state=None, **kwargs):
        ...
        if self.dialogue_encoder_gating == "GRU":
            ...
            o_hier_info = [hs_0, None, None, None, None, None, None, None, None, None, None, None, None, None, None,
        ...
        if self.normop_type == 'BN':
            if self.dialogue_encoder_gating == "GRU":
                if (not one_step) and (h.ndim == 3) and (bnmask_given):
                    vars_to_update = [self.normop_rs_h_mean, self.normop_rs_h_var, self.normop_rs_hs_mean,
                                      self.normop_rs_hs_var, self.normop_zs_h_mean, self.normop_zs_h_var,
                                      self.normop_zs_hs_mean, self.normop_zs_hs_var, self.normop_os_h_mean,
                                      self.normop_os_h_var, self.normop_os_hs_mean, self.normop_os_hs_var,
                                      self.normop_in_h_mean, self.normop_in_h_var, self.normop_in_hs_mean,
                                      self.normop_in_hs_var]
...
class UtteranceDecoder(EncoderDecoderBase):
    ...
    def init_params(self):
        ...
        if self.utterance_decoder_gating == "" or self.utterance_decoder_gating == "NONE" \
            ...
            self.Wd_hh = add_to_params(self.params, theano.shared(
                value=OrthogonalInit(self.rng, self.qdim_decoder, self.qdim_decoder), name='Wd_hh'))
            self.bd_hh = add_to_params(self.params, theano.shared(value=np.zeros((self.qdim_decoder,), dtype='float32'),
                                                                  name='bd_hh'))
        ...
        if self.utterance_decoder_gating == "GRU":
            ...
            self.Wd_bow_W_in = add_to_params(self.params, theano.shared(
                value=NormalInit(self.rng, self.input_dim, self.qdim_decoder), name='Wd_bow_W_in'))
            self.Wd_bow_b_in = add_to_params(self.params,
                                             theano.shared(value=np.zeros((self.qdim_decoder,), dtype='float32'),
                                                           name='Wd_bow_b_in'))
            ...
            if self.utterance_decoder_gating == "" or self.utterance_decoder_gating == "NONE" \
                ...
                self.Wd_e_out = add_to_params(self.params,
                                              theano.shared(value=NormalInit(self.rng, self.rankdim, out_target_dim),
                                                            name='Wd_e_out'))
                self.bd_e_out = add_to_params(self.params,
...
class DialogLevelLatentGaussianEncoder(EncoderDecoderBase):
    ...
    def init_params(self):
        ...
        self.input_mlp = TwoLayerMLP(self.state, self.rng, self.input_dim, self.latent_dim * 2, self.latent_dim, self,
                                     '_input_mlp_' + self.name)
        self.params += self.input_mlp.params
    ...
    def build_encoder(self, h, x, xmask=None, latent_variable_mask=None, prev_state=None, **kwargs):
        ...
        hs_var = T.nnet.softplus(
            (T.dot(hs, self.Wl_std_out) + self.bl_std_out)) * self.scale_latent_gaussian_variable_variances

        hs_var = T.clip(hs_var, self.min_latent_gaussian_variable_variances,
                        self.max_latent_gaussian_variable_variances)
...
class DialogEncoderDecoder(Model):
            ...
            if self.add_latent_gaussian_per_utterance:
                self.train_fn = theano.function(inputs=[self.x_data, self.x_data_reversed,
                                                ...
                                                outputs=[self.training_cost, self.kl_divergence_cost_acc,
                                                         self.latent_gaussian_utterance_variable_approx_posterior_mean_var,
            ...
            if self.add_latent_gaussian_per_utterance:
                self.eval_fn = theano.function(
                    inputs=[self.x_data, self.x_data_reversed, self.x_max_length, self.x_cost_mask, self.x_reset_mask,
                            self.ran_gaussian_cost_utterance, self.ran_uniform_cost_utterance, self.x_dropmask],
        ...
        if not hasattr(self, 'next_probs_fn'):
            ...
            if self.add_latent_gaussian_per_utterance:
                ...
                if self.condition_latent_variable_on_dialogue_encoder:
                    ...
                    hs_to_condition_latent_variable_on = T.alloc(np.float32(0), self.beam_hs.shape[0], 1,
                ...
                if self.add_latent_gaussian_per_utterance:
                    ...
                    _gaussian_prior_out, _ = self.latent_gaussian_utterance_variable_prior_encoder.build_encoder(
                        hs_to_condition_latent_variable_on, self.beam_x_data[-1])
                    ...
                    prior_gaussian_sample = self.beam_ran_gaussian_cost_utterance * T.sqrt(
                        latent_gaussian_utterance_variable_prior_var) + latent_gaussian_utterance_variable_prior_mean
            ...
            self.next_probs_fn = theano.function(inputs=[self.beam_hs, self.beam_hd, self.beam_source, self.beam_x_data,
    ...
    def build_encoder_function(self):
        if not hasattr(self, 'encoder_fn'):
            ...
            if self.add_latent_gaussian_per_utterance:
                ...
                platent_gaussian_utterance_variable_approx_posterior = theano.shared(
                    value=numpy.zeros((self.bs, self.latent_gaussian_per_utterance_dim), dtype='float32'),
                    name='encoder_fn_platent_gaussian_utterance_variable_approx_posterior')
                ...
                # Compute prior
                _prior_out, _ = self.latent_gaussian_utterance_variable_prior_encoder.build_encoder(
                    hs_to_condition_latent_variable_on, self.x_data, latent_variable_mask=latent_variable_mask)
                ...
                latent_utterance_variable_approx_posterior_mean = self.gaussian_posterior_mean_combination.build_output(
                    self.hs_and_h_future, _prior_out[1], _posterior_out[1])
                latent_utterance_variable_approx_posterior_var = self.posterior_variance_combination.build_output(
                    self.hs_and_h_future, _prior_out[2], _posterior_out[2])
    ...
    def __init__(self, state):
        ...
        if self.add_latent_gaussian_per_utterance:
            self.platent_gaussian_utterance_variable_prior = theano.shared(
                value=numpy.zeros((self.bs, self.latent_gaussian_per_utterance_dim), dtype='float32'),
                name='platent_gaussian_utterance_variable_prior')
            self.platent_gaussian_utterance_variable_approx_posterior = theano.shared(
                value=numpy.zeros((self.bs, self.latent_gaussian_per_utterance_dim), dtype='float32'),
                name='platent_gaussian_utterance_variable_approx_posterior')
        ...
        if self.bidirectional_utterance_encoder:
            ...
            res_forward, res_forward_n, res_forward_updates = self.utterance_encoder_forward.build_encoder(training_x,
            ...
            res_backward, res_backward_n, res_backward_updates = self.utterance_encoder_backward.build_encoder(
                training_x_reversed, xmask=training_hs_mask, prev_state=[self.ph_bck, self.ph_bck_n])
            ...
            res_forward, res_forward_n, res_forward_updates = self.utterance_encoder.build_encoder(training_x,
                                                                                                   xmask=training_hs_mask,
                                                                                                   prev_state=[self.ph,
                                                                                                               self.ph_n])
        ...
        self.hs, self.dialogue_encoder_updates = self.dialog_encoder.build_encoder(self.h, training_x,
                                                                                   xmask=training_hs_mask,
                                                                                   prev_state=self.phs)
        ...
        if self.add_latent_gaussian_per_utterance:
            ...
            self.latent_gaussian_utterance_variable_prior_encoder = DialogLevelLatentGaussianEncoder(self.state,
                                                                                                     self.sdim,
                                                                                                     self.latent_gaussian_per_utterance_dim,
                                                                                                     self.rng, self,
                                                                                                     'latent_gaussian_utterance_prior')

            logger.debug("Build prior encoder for utterance-level latent multivariate Gaussian variables")
            _prior_gaussian_out, _prior_gaussian_updates = self.latent_gaussian_utterance_variable_prior_encoder.build_encoder(
                self.hs_to_condition_latent_variable_on, training_x, xmask=training_hs_mask,
                latent_variable_mask=latent_variable_mask, prev_state=self.platent_gaussian_utterance_variable_prior)
            ...
            self.latent_gaussian_utterance_variable_approx_posterior_encoder = DialogLevelLatentGaussianEncoder(
                self.state, posterior_input_size, self.latent_gaussian_per_utterance_dim, self.rng, self,
                'latent_gaussian_utterance_approx_posterior')

            logger.debug(
                "Build approximate posterior encoder for utterance-level latent multivariate Gaussian variables")
            ...
            self.gaussian_posterior_mean_combination = LinearCombination(self.state, posterior_input_size,
                                                                         self.latent_gaussian_per_utterance_dim, False,
                                                                         0.0, 0.0, self.rng, self,
                                                                         'latent_gaussian_utterance_approx_posterior_mean_combination')
            self.latent_gaussian_utterance_variable_approx_posterior_mean = self.gaussian_posterior_mean_combination.build_output(
                self.hs_and_h_future, self.latent_gaussian_utterance_variable_prior_mean, _posterior_gaussian_out[1])

            # Use an MLP to interpolate between prior variance and candidate posterior variance.
            # This allows model to revert back to prior easily for dimensions, where it is uncertain.
            self.posterior_variance_combination = LinearCombination(self.state, posterior_input_size,
                                                                    self.latent_gaussian_per_utterance_dim, True,
                                                                    self.min_latent_gaussian_variable_variances,
                                                                    self.max_latent_gaussian_variable_variances,
                                                                    self.rng, self,
                                                                    'latent_gaussian_utterance_approx_posterior_variance_combination')
            self.latent_gaussian_utterance_variable_approx_posterior_var = self.posterior_variance_combination.build_output(
                self.hs_and_h_future, self.latent_gaussian_utterance_variable_prior_var, _posterior_gaussian_out[2])

            self.latent_gaussian_utterance_variable_approx_posterior_mean_var = T.sum(
                T.mean(self.latent_gaussian_utterance_variable_approx_posterior_var, axis=2) * latent_variable_mask) / (
                                                                                            T.sum(
                                                                                                latent_variable_mask) + 0.0000001)

            # Sample utterance latent variable from posterior
            self.latent_gaussian_posterior_sample = self.ran_gaussian_cost_utterance[:(self.x_max_length - 1)] * T.sqrt(
                self.latent_gaussian_utterance_variable_approx_posterior_var) + self.latent_gaussian_utterance_variable_approx_posterior_mean
        ...
        if self.direct_connection_between_encoders_and_decoder:
            ...
            if self.add_latent_gaussian_per_utterance:
                if self.condition_decoder_only_on_latent_variable:
                    ...
                    self.hd_input = T.concatenate([self.hs, self.hs_dummy, self.latent_gaussian_posterior_sample],
                                                  axis=2)
            else:
        ...
        target_probs, self.hd, target_probs_full_matrix = self.utterance_decoder.build_decoder(self.hd_input,
                                                                                               training_x,
                                                                                               xmask=training_hs_mask,
                                                                                               xdropmask=training_x_dropmask,
                                                                                               y=training_y,
        ...
        if self.add_latent_gaussian_per_utterance:
            ...
            assert len(set(self.params)) + len(
                set(self.latent_gaussian_utterance_variable_approx_posterior_encoder.params)) \
        ...
        self.updates, self.optimizer_variables = self.compute_updates(self.training_cost / training_x.shape[1],
                                                                      self.params_to_train)
        ...
        if self.add_latent_gaussian_per_utterance:
            self.state_updates.append((self.platent_gaussian_utterance_variable_prior,
                                       x_reset * self.latent_gaussian_utterance_variable_prior[-1]))
            self.state_updates.append((self.platent_gaussian_utterance_variable_approx_posterior,
                                       x_reset * self.latent_gaussian_utterance_variable_approx_posterior[-1]))
